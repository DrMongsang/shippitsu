...
### 8.6 データ共有
✅ **全店舗でデータ共有**
- 1つのデータベースで全店舗のデータを管理
- フィルター機能で自店舗のデータだけ表示可能
- 顧客の他店舗でのお直し履歴も確認できる
...
- CSチームへの確認フローは実装しない
- ただし、**テンプレート選択機能を実装**:
  - 備考欄・原因分析欄などでよく使う定型文を選択可能
  - 例: 「お客様のご要望による変更」「体型変化による調整」「ヒアリング不足」など
  - プルダウンまたはボタンで選択して自動入力

### 8.8 サポート外お直し費用
✅ **サポート内/サポート外の選択**
- お直し登録時に「サポート内 / サポート外」を選択
- **サポート内**: 料金 = ¥0（無料表示）
- **サポート外**: 通常通り料金計算（箇所数×単価）
- サポート外の詳細な計算ルールはPhase 2で拡張
...
- 登録時は自動的に「対応中」
- 完了ボタンで「完了」に変更
- より詳細なステータス（工場発送済みなど）はPhase 2で検討
